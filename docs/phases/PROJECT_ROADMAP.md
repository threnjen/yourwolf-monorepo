...
| 05b | Local Role Authoring | Planned | Phase 05a | Small | Role validation and warnings ported to `src/domain/` with the Python rule tests transcribed as the oracle. The server name check is removed in favor of the 05a local check. The HTTP client, Axios, and the test guard are deleted. The frontend makes zero server calls. |
