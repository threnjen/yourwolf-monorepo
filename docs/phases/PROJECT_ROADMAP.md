...
| 05a | Local Catalog and Store | Implementation Complete | Phase 04b | Medium | Repository interface with one IndexedDB implementation. Bundled seed roles and abilities seed on first launch with versioned reseed. Role list, role detail, abilities, role save, and game snapshots use the local store. Browser manual QA remains pending; the packaged-origin check belongs to Phase 06. |
| 05b | Local Role Authoring | Planned | Phase 05a | Small | Role validation, warnings, and name check ported to the domain with a Python parity fixture. The HTTP client is deleted. The frontend makes zero server calls. |
