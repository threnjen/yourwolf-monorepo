...
| 05a | Local Catalog and Store | Complete | Phase 04b | Medium | Repository interface with one IndexedDB implementation. Bundled seed roles and abilities seed on first launch with versioned reseed. Role list, role detail, abilities, role save, and game snapshots use the local store. The 31 browser manual QA rows in `PHASE_05A/PHASE_05A_QA.md` were not executed at close; the packaged-origin check belongs to Phase 06a. |
| 05b | Local Role Authoring | Complete | Phase 05a | Small | Role validation and warnings run in `src/domain/` with transcribed Python rule tests as the oracle. Name checks use the local catalog. The HTTP client and Axios are absent, and offline guards prove the frontend makes zero server calls. Manual browser QA remains pending in `PHASE_05B/PHASE_05B_QA.md`. |
