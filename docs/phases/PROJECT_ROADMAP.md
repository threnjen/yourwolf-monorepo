...
| 05a | Local Catalog and Store | Planned | Phase 04b | Medium | Repository interface with one IndexedDB implementation. Bundled seed roles and abilities seeded on first launch with versioned reseed. Role list, role detail, abilities, role save, and game snapshots on the local store. Local name uniqueness check. |
