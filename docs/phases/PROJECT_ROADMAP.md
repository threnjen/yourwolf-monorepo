...
| 04b | Engine Frontend Integration | Planned | Phase 04a | Medium | Replace the six game-flow and preview API call sites with the local engine; persist the in-progress game in session storage so navigation and refresh work; fetch role detail per selected role at Start Game; delete the games API client; run end-to-end manual QA |
